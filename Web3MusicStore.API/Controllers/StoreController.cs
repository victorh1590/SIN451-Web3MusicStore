...
public class StoreController : Controller
{
  ...
  public async Task<IActionResult> GetTrackList(string album_id)
  {
    ...
    albumView.Add(converter.AlbumViewModelConverter(item));

    var songs = await _repository.Songs
        .Select(x => x)
        .Where(x => x.id_artists.Contains(albumView.First().artists_view.Keys.First().ToString()))
        .Take(5)
        .ToListAsync();
